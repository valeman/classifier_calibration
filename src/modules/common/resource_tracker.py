...
class ResourceTracker:
    ...
    def _sample_memory(self):
        while not self._stop_event.is_set():
            try:
                self.max_r_memory = self._read_memory(
                    self.max_r_memory, "memory.current"
                )
                self.max_s_memory = self._read_memory(
                    self.max_s_memory, "memory.swap.current"
                )
                self.max_zs_memory = self._read_memory(
                    self.max_zs_memory, "memory.zswap.current"
                )
    ...
    def __enter__(self):
        ...
        if self.cgroup_path in ["/sys/fs/cgroup/", "/sys/fs/cgroup/user.slice/"]:
            lg.warning(
                "Not running in an isolated cgroup. Use systemd-run --user --scope."
            )
    ...
    def __exit__(self, exc_type, exc, tb):
        ...
        # Collect results
        b_to_mib = 1024**2
        self.total_cpu_time_sec = (
            t_cpu_end - self.t_cpu_start
        ) / 1_000_000  # convert usec to sec
        ...
        self.peak_memory_mib = (
            self.max_r_memory / b_to_mib
            if not isinstance(self.max_r_memory, str)
            else self.max_r_memory
        )
        self.peak_swap_mib = (
            self.max_s_memory / b_to_mib
            if not isinstance(self.max_s_memory, str)
            else self.max_r_memory
        )
        self.peak_zswap_mib = (
            self.max_zs_memory / b_to_mib
            if not isinstance(self.max_zs_memory, str)
            else self.max_r_memory
        )
