...
class Architecture:
    ...
    def fit(self, meta_data: dict, x_train: pd.DataFrame, y_train: pd.Series) -> None:
        ...
        if not (self.post_processing is None):
            y_cal_prob = self.learner.predict_prob(
                x_calibration if self.calibration_set else x_train
            )
            ...
            self.post_processing.fit(
                y_cal_prob, y_calibration if self.calibration_set else y_train
            )
    ...
    def init_v1(self) -> None:
        ...
        for l in learners:
            for p in phc:
                if p is None:
                    archs.append(
                        Architecture(name=l.learner_name, learner=l, random_seed=SEED)
                    )
                else:
                    archs.append(
                        Architecture(
