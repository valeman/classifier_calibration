...
class PerformanceMeasures:
    ...
    def init_v1(self) -> None:
        ...
        self.measure_functions["brier_score"] = (
            lambda x_test, y_prob, y_pred, y_test: ce(y_test, y_prob).brier
        )
        self.measure_functions["abs_clip_spiegelhalter_z_statistic"] = (
            lambda x_test, y_prob, y_pred, y_test: abs(
                np.clip(ce(y_test, y_prob).z_test().statistic, a_min=-12, a_max=12)
            )
        )
        self.measure_functions["log_loss"] = (
            lambda x_test, y_prob, y_pred, y_test: skm.log_loss(
                y_test, y_prob, normalize=True
            )
        )

        self.measure_functions["auc_roc"] = lambda x_test, y_prob, y_pred, y_test: ce(
            y_test, y_prob
        ).auroc

        self.measure_functions["accuracy"] = (
            lambda x_test, y_prob, y_pred, y_test: skm.accuracy_score(
                y_test, y_pred, normalize=True
            )
        )
        self.measure_functions["recall_1"] = (
            lambda x_test, y_prob, y_pred, y_test: skm.recall_score(
                y_test, y_pred, pos_label=1, average="binary"
            )
        )
        self.measure_functions["recall_0"] = (
            lambda x_test, y_prob, y_pred, y_test: skm.recall_score(
                y_test, y_pred, pos_label=0, average="binary"
            )
        )
        self.measure_functions["f1_score"] = (
            lambda x_test, y_prob, y_pred, y_test: skm.f1_score(
                y_test, y_pred, pos_label=1, average="binary"
            )
        )
        self.measure_functions["precision"] = (
            lambda x_test, y_prob, y_pred, y_test: skm.precision_score(
                y_test, y_pred, pos_label=1, average="binary"
            )
        )
        ...
        def calfram_measures(measure_name, y_test, y_prob, y_pred):
            ...
            classes_scores = caf.select_probability(
                y_test,  # shape: (n,)
                y_prob,  # shape: (n, c), where c is the number of classes
                y_pred,  # shape: (n,)
            )

            measures, _ = caf.calibrationdiagnosis(
                classes_scores,
                adaptive=True,  # Binning by automatic monotonic sweep method
            )
        ...
        self.measure_functions["eci_global"] = (
            lambda x_test, y_prob, y_pred, y_test: calfram_measures(
                "eci_global", y_test, y_prob, y_pred
            )
        )
        self.measure_functions["eci_balance"] = (
            lambda x_test, y_prob, y_pred, y_test: calfram_measures(
                "eci_balance", y_test, y_prob, y_pred
            )
        )
        self.measure_functions["ece_freq"] = (
            lambda x_test, y_prob, y_pred, y_test: calfram_measures(
                "ece_freq", y_test, y_prob, y_pred
            )
        )
    ...
    def qc_binary_input(
        ...
        for name, arr in (("y_prob", y_prob), ("y_pred", y_pred), ("y_test", y_test)):
            ...
            if arr.shape[0] != n:
                raise ValueError(
                    f"Length mismatch: len(x_test)={n} but {name}.shape[0]={arr.shape[0]}"
                )
        ...
        if not np.all((y_prob >= 0) & (y_prob <= 1)):
            ...
            raise ValueError(
                f"y_prob contains values outside [0,1] at indices {bad_idx.tolist()}"
            )
        ...
        for name, arr in (("y_pred", y_pred), ("y_test", y_test)):
            ...
            if bad:
                raise ValueError(
                    f"{name} contains non‐binary values {bad}; only {{0,1}} allowed"
                )
