...
def encode_categoricals(
    ...
    for col in cat_cols:
        ...
        df_encoded[col] = pd.Categorical(
            df_encoded[col].map(m), categories=list(m.values())
        )
...
class Dataset:
    ...
    def pre_process(self, method: str) -> None:
        ...
        match method:
            case "detect_categorical":
                detected_cat_cols = detect_categorical_columns(
                    self.df[self.non_cat_columns], random_seed=self.random_seed
                )
            ...
            case "convert_nan_to_unique_val":  # only for cat columns
                ...
                for col in include_columns:
                    ...
                    n_unique = util.get_unique_id(
                        uniques, pre_fix="nan_", random_seed=self.random_seed
                    )
    ...
    def _update_categorial_meta_data(self, detected_cat_cols: list[str]) -> None:
        ...
        cat_features_indices = [
            i
            for i, c in enumerate(self.df[features].columns)
            if c in self.meta_data["cat_features"]
        ]
