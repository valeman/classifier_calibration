...
class EvaluationStrategy:
    ...
    def __init__(
        ...
        strategy: str,
        ds: Dataset,
        p_measures: PerformanceMeasures,
        random_seed: int = 123,
    ):
        ...
        for x_train, y_train, x_test, y_test in self.gen_sets():
            ...
            with ResourceTracker(sample_interval=0.05) as rt:
                util.run_with_timeout(
                    max_time_sec, arch.fit, self.ds.meta_data, x_train, y_train
                )
            ...
            perf_measures = self.p_measures.calc_perf(
                x_test, y_prob, y_pred, np.asarray(y_test)
            )
            perf_measures = self.log_computational_performance(
                "fit", perf_measures, time_fit, rsc_fit
            )
            perf_measures = self.log_computational_performance(
                "pre", perf_measures, time_pre, rsc_pre
            )
            ...
            lg.info(
                f"End run {count}. Wall time spent: {util.format_time(run_end - run_start)}"
            )
            count += 1
    ...
    def k_fold_CV(self, ds: Dataset, k=5) -> Generator[tuple[pd.DataFrame], None, None]:
        ...
        def gen_sets():
            # Stratified split
            cv = StratifiedKFold(
                n_splits=k, shuffle=True, random_state=self.random_seed
            )
    ...
    def log_computational_performance(
        self, section: str, perf_measures: dict, timed: float, rsc: dict
    ) -> dict:
