...
def rank_abs_arch(
    inv_res: dict, ranking_m: dict, agg_key: str, n_runs: int, archs: list, n_archs: int
):
    ...
    ranking_dict = rank_across_runs_by_me(
        agg_key, ranking_m, ranking_dict, archs, n_archs
    )
...
def rank_del_phcms(
    delta_inv_res: dict, ranking_m: dict, agg_key: str, n_runs: int, archs: list
):
    ...
    ranking_dict = delta_rank_across_runs_by_me_lr(
        agg_key, ranking_m, ranking_dict, archs
    )
...
def rank_across_runs_by_ds_me(
    inv_res: dict, ranking_m: dict, n_runs: int, archs: list, n_archs: int
) -> dict:
    """
        ...
        for measure, desc in ranking_m.items():
            ...
            for i in range(n_runs):
                ...
                for arch_name in archs:
                    m_vals.append(round(arch_dict[arch_name][i][measure], 5))
...
def rank_across_runs_by_ds_me_lr(
    d_inv_res: dict, ranking_m: dict, n_runs: int, archs: list
) -> dict:
    """
    ...
    for ds_name, arch_dict in d_inv_res.items():
        ranking_dict[ds_name] = {}
        for measure, desc in ranking_m.items():
            ranking_dict[ds_name][measure] = {
                l: {p: np.zeros(len(phcms)) for p in phcms} for l in lrns
            }
            for lrn in lrns:
                for i in range(n_runs):
                    m_vals = []
                    for phcm in phcms:
                        m_vals.append(round(arch_dict[f"{lrn}.{phcm}"][i][measure], 5))
...
def rank_across_runs_by_me(
    agg_key: str, ranking_m: dict, ranking_dict: dict, archs: list, n_archs: int
) -> dict:
    """
    ...
    """
    ranking_dict[agg_key] = {
        k: {a: np.zeros(n_archs) for a in archs} for k in ranking_m.keys()
    }
...
def delta_rank_across_runs_by_me_lr(
    agg_key: str, ranking_m: dict, ranking_dict: dict, archs: list
) -> dict:
    """
    ...
    phcms = get_phcm(archs)
    ranking_dict[agg_key] = {
        m: {l: {p: np.zeros(len(phcms)) for p in phcms} for l in lrns}
        for m in ranking_m
    }
...
def delta_rank_across_runs_by_me(
    agg_key: str, ranking_m: dict, ranking_dict: dict, archs: list
) -> dict:
    """
    ...
    for measure, m_dict in ranking_dict[agg_key].items():
        ranking_dict[agg_key][measure][agg_key] = {
            p: np.zeros(len(phcms)) for p in phcms
        }
...
def rank_across_runs(
    agg_key: str, ranking_dict: dict, archs: list, n_archs: int
) -> dict:
    """
