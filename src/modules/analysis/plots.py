...
def plot_scatter_cc(
    inv_res: dict, ds_md: dict, archs: list, cc_m: str, outfile: str
) -> None:
    """
    #TODO: FILL
    """
    ...
    for arch in archs:
        ...
        for ds_name, ds_dict in inv_res.items():
            if "fit" in cc_m:
                n_inst = round(
                    ds_md[ds_name]["n_rows"] * 4 / 5
                )  # TODO: Pass down from main
            else:
...
def plot_box_cu(res: dict, archs: list, cu_m: str, output_path: str) -> None:
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=(4 * n_cols, 3 * n_rows), squeeze=False, sharey=True
    )
...
def plot_dual_rankings(
    data: dict, archs: list, outfile: str, meas_1: str, meas_2: str, monkey_fix=False
) -> None:
    # Extract unique base models and PHCMs
    base_models = sorted({arch.split(".")[0] for arch in archs})
    phcms = sorted(
        {"none" if "." not in arch else arch.split(".", 1)[1] for arch in archs}
    )
    ...
    phcm_marker_map = {
        phcm: all_markers[i % len(all_markers)] for i, phcm in enumerate(phcms)
    }
    ...
    ax.xaxis.set_major_locator(
        plt.MaxNLocator(nbins=len(archs), steps=[2, 4], integer=True)
    )
...
def find_bin_edges(data: dict, d_type: str, center: float, max_iter=1000):
    ...
    while iter_count < max_iter:
        ...
        right_edges = np.arange(
            center, interval + step, step
        )  # +step to include endpoint
...
def plot_centered_stacked_hist(
    data: dict,
    group: str,
    meas: str,
    d_type: str,
    ...
    n_phc = len(data)
    fig, axes = plt.subplots(
        n_phc, 1, figsize=(12, max(2.5 * n_phc, 4)), sharex=True, squeeze=False
    )
    axes = axes.flatten()
    ...
    core_ticks = [
        ...
        and not any(
            abs(t - s) < proximity_threshold  # Avoid special ticks
...
def plot_changes(
    d_inv_res: dict, measures_d: dict, archs: list, dir_path: str, d_type: str
) -> None:
    lrns = get_learners(archs)
    phcms = get_phcm(archs)
    ...
    for meas, more_is_better in measures_d.items():
        ...
        for lrn in lrns:
            ...
            plot_centered_stacked_hist(
                lrn_values, lrn, meas, d_type, more_is_better, path
            )

        plot_centered_stacked_hist(
            all_values, "all", meas, d_type, more_is_better, phcms_dir
        )
