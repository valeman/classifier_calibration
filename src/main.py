...
if __name__ == "__main__":
    ...
    datasets_metadata = {}
    experiment_metadata = {
        "machine_md": {"n_cores": n_cores, "max_ram_mib": util.get_max_ram_mib()},
        "random_seed": SEED,
    }
    results = {}
    ...
    with progress:
        ...
        for ds_idx, ds in enumerate(datasets, start=1):
            ...
            ds.pre_process(
                "convert_nan_to_unique_val"
            )  # Replace nan in cat columns with a new uniqe value
            ds.pre_process("encode_categoricals")  # To {0,1} if binary else {1,2,3,...}
            ds.pre_process(
                "clean_numerical"
            )  # Ensure non-cat object columns only contain numbers.
            ...
            for arch_idx, arch in enumerate(architectures, start=1):
                ...
                eval_strat = EvaluationStrategy(
                    eval_strat_name, ds, p_measures, random_seed=SEED
                )
                try:
                ...
                lg.info(
                    f"End evaluation of {arch.name}. Wall time spent: {util.format_time(end_eval - start_eval)}"
                )

            end_ds = time.perf_counter()
            lg.info(
                f"All evaluations on {ds.df_name} ended. Wall time spent: {util.format_time(end_ds - start_ds)}"
            )
            progress.remove_task(inner)

    end_exp = time.perf_counter()
    lg.info(
        f"Experiment completed. Wall time spent {util.format_time(end_exp-start_exp)}"
    )
