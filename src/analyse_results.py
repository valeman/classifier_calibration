from modules.analysis.ranking import (
    rank_abs_arch,
    rank_del_phcms,
    get_learners,
    get_phcm,
)
from modules.analysis.plots import (
    plot_scatter_cc,
    plot_box_cu,
    plot_rankings,
    plot_changes,
            ...
            for i, run in enumerate(runs):
                ...
                for measure, value in run.items():
                    ...
                    if not util.all_numbers_and_finite(np.asarray([value])):
                        lg.info(
                            f"ValueError: Run {i} ({arch} on {ds}): {measure!r}={value!r} is not a finite number"
                        )

                    eval = scs[measure](value)
                    if eval:
                        lg.info(
                            f"ValueError: Run {i} ({arch} on {ds}): {measure!r}={value!r}  Value {eval}"
                        )
        ...
        for ds_name, runs in ds_dict.items():
            for run in runs:
                for stage in ["fit", "pre"]:
                    ...
                    run[f"cpu_time_total_{stage}_util_pct"] = (
                        run[f"cpu_time_total_{stage}_sec"] / max_cpu_time_total_sec
                    ) * 100
                    run[f"peak_ram_{stage}_util_pct"] = (
                        run[f"peak_ram_{stage}_mib"] / max_ram
                    ) * 100
    return res
        ...
        for learner in learners:
            ...
            for pp_a in pp_as:
                marg_inv_res[ds_name][pp_a] = [
                    {k: p[k] - b[k] for k in b if k != "status"}
                    for b, p in zip(ds_dict[learner], ds_dict[pp_a])
                ]
    return marg_inv_res
...
def rank_archs(
    ...
    _, exp_ranking_dict = rank_abs_arch(
        inv_res, ranking_m, agg_key, n_runs, archs, n_archs
    )
    ...
    for ms_name in exp_ranking_dict[agg_key].keys():
        plot_rankings(
            exp_ranking_dict[agg_key][ms_name],
...
def rank_phcms(
    ...
    _, exp_ranking_dict = rank_del_phcms(
        delta_inv_res, ranking_m, agg_key, n_runs, archs
    )
    ...
    for ms_name in exp_ranking_dict[agg_key].keys():
        ...
        for lrn in exp_ranking_dict[agg_key][ms_name]:
            title_str = (
                f"Expected performance rank across folds and datasets"
                if lrn != agg_key
                else f"Expected performance rank across models, folds, and datasets"
            )
            plot_rankings(
                exp_ranking_dict[agg_key][ms_name][lrn],
    ...
    m_vals = {
        k: v[agg_key] for k, v in exp_ranking_dict[agg_key].items() if k != agg_key
    }
    i_dir_path = util.create_pwd_dir(dir_path + post_fix + "/dual/")
...
def analyse_results(
    res: dict, ds_md: dict, exp_md: dict, output_dir: str, assets_dir: str
) -> None:
    """
    Args:
    ...
    rank_archs(
        inv_res,
        ranking_m | comp_cost_m,
        agg_key,
        dir_path,
        n_runs,
        archs,
        n_archs,
        post_fix="/abs/rank/archs/",
    )
    rank_archs(
        inv_res_lrns,
        ranking_m | comp_cost_m,
...
if __name__ == "__main__":
    ...
    # lg.info(f"Merging experiment results from: {output_dir}")
